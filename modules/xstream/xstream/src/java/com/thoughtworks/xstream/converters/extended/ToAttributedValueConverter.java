 ...
 * written as value and all other field members are written as attributes. The
 * converter requires that all the field types (expect the one with the value)
 * are handled by a {@link SingleValueConverter}. The value field is defined
 * using the name of the type that declares the field and the field name itself.
 * Therefore it is possible to define an inherited field as value. It is also
 * possible to provide no value field at all, so that all fields are written as
 * attributes.
 * 
...
public class ToAttributedValueConverter implements Converter {
	 ...
	 * @since 1.4.9
	 */
	public ToAttributedValueConverter(final Class type, final Mapper mapper,
	...
	public Object unmarshal(final HierarchicalStreamReader reader, final UnmarshallingContext context) {
		...
		if (valueField != null) {
			...
			if (classAttribute != null) {
				...
				type = mapper.defaultImplementationOf(
